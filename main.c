...
// Outputs: None
void WaitForASLow(void){
	while (GPIO_PORTF_DATA_R & 0x10);			// stay in loop while SW1 is not pressed
}
...
// Outputs: None
void WaitForASHigh(void){
	while (!(GPIO_PORTF_DATA_R & 0x10));	// stay in loop while SW1 is pressed
}
...
void Delay1ms(unsigned long msec){
	...
	while (msec > 0) {
		count = 13333;							// takes around 1ms when clock is 80 MHz
